...
#on Illumina lowercase letters represent high quality data
```
...
#links through the cbrg server are broken can add Bioconductor directly to my computer and see Vignettes. 
...
```{r}
library(BSgenome.Hsapiens.UCSC.hg38.masked)
#masked ha hidden all of the genomic repeats that are difficult to map
```

- Assign the genome object provided in the package to a new object called `genome`.

```{r}
genomehg38masked <- BSgenome.Hsapiens.UCSC.hg38.masked
genomehg38masked
```
...
seqnames(genomehg38masked)
chrY <- genomehg38masked$chrY
...
```{r}
Ebox <- "CANNTG" # unnessesary but could assign say the GATA1 binding sites are a function.
countPattern("CANNTG",  chrY, fixed=F)
countPattern(Ebox,  chrY, fixed=F) #alternative
```
...
```{r}
unique_trans<-unique(gtf_ACTB$transcript_id)
unique_trans<-na.omit(unique_trans)
length(unique_trans)
```
...
# when need to search move the console and type"?" then the function wanting to use eg. ?split ?unique ?subset
...
```{r}
#keys are the values in the database
...
```{r}
listMarts()
#lists what is available today on the BioMart webpage
```
...
```{r}
  #listFilters(mart) to find the list we can filter with within the mart.
...
assays(demo_se) 
assayNames(demo_se)
assay(demo_se, "counts") # to see the matrix
colData(demo_se)
rowData(demo_se)
mcols(demo_se) #same as rowData don't use this here
metadata(demo_se) #no information stored here for this experiment
