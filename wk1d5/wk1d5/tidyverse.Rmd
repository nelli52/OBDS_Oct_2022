...
sepal_length_med= median(sepal_length),
...
exprs_assay <- read_xlsx(path ="/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet="exprs")
...
exprs_assay_long <- exprs_assay %>% 
    pivot_longer(!gene, names_to = "sampleID", values_to = "exp"  )
#(!species, names_to="variable", values_to = "value" )
exprs_assay_long
```
...
```{r}
ggplot(exprs_assay_long, aes(x=exp, fill=gene)) +
geom_histogram()+facet_wrap(~gene)
#ggplot(iris_for_ggplot,aes(x=sepal_area, fill=species)) +geom_histogram()+facet_wrap(~species, nrow=2)
```
...
library(readxl)
excel_sheets("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx")
sample_info <- read_xlsx(path ="/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet="sample_info")
sample_info
...
exprs_sampleinfo <- sample_info %>%
    inner_join(exprs_assay_long,by = c("unique_patient_ID"="sampleID"))
exprs_sampleinfo
```
...
```{r}
  ...
  pivot_wider(names_from = sex, values_from = exp) %>% 
